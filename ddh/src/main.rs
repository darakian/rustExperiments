...
fn main() {
    ...
    println!("{:?} Shared files: {:?} {}", common_files.len(), common_files.iter().fold(0, |sum, x| sum+x.file_len)/display_divisor, blocksize);
    match arguments.value_of("Print").unwrap_or(""){
        ...
        "S" => {println!("Shared Files"); common_files.iter().for_each(|x| println!("{:?}", x.file_paths[0].to_str().unwrap()));},
        _ => {}};
